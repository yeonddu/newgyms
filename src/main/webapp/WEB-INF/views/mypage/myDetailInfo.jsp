    ...
    function execDaumPostcode() {
        new daum.Postcode({
            oncomplete: function(data) {
                ...
                var fullRoadAddr = data.road_address; // 도로명 주소 변수
                ...
                document.getElementById('jibun_address').value = data.jibun_address;

                // 사용자가 '선택 안함'을 클릭한 경우, 예상 주소라는 표시를 해준다.
                if(data.autoroad_address) {
                    //예상되는 도로명 주소에 조합형 주소를 추가한다.
                    var expRoadAddr = data.autoroad_address + extraRoadAddr;
                    document.getElementById('guide').innerHTML = '(예상 도로명 주소 : ' + expRoadAddr + ')';

                } else if(data.autojibun_address) {
                    var expJibunAddr = data.autojibun_address;
...
function fn_modify_member_info(attribute){
	...
	// alert("mod_type:"+mod_type);
		...
		if(attribute=='member_pw'){
			...
			var road_address=frm_mod_member.road_address;
			var jibun_address=frm_mod_member.jibun_address;
			var namuji_address=frm_mod_member.namuji_address;
			
			value_zipcode=zipcode.value;
			value_road_address=road_address.value;
			value_jibun_address=jibun_address.value;
			value_namuji_address=namuji_address.value;
			value=value_zipcode+","+value_road_address+","+value_jibun_address+","+value_namuji_address;
		}
			...
			<tbody>
				...
				<tr class="dot_line">
					...
					<td>
					  ...
					  <br>
					  <p> 
					   지번 주소:<br><input type="text" id="road_address"  name="road_address" size="50" value="${memberInfo.road_address }"><br><br>
					  도로명 주소: <input type="text" id="jibun_address" name="jibun_address" size="50" value="${memberInfo.jibun_address }"><br><br>
					  나머지 주소: <input type="text"  name="namuji_address" size="50" value="${memberInfo.namuji_address }" />
					   </p>
					</td>
					<td>
