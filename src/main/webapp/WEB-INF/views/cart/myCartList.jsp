		...
		$('.modify_option_btn').on('click', function() { 
			...
			$.ajax({
				...
				success : function(data) {
					//var result = JSON.stringify(data); //JSON 형태의 문자열(string)으로 변환
					//console.log(result)
					
					//var optListJson = JSON.parse(result);
					//console.log(optListJson)
					var data_length = Object.keys(data).length; //JSON 객체의 길이 구하기
					console.log("data_length: "data_length);
					
					console.log("data[0].product_option_name: " + data[0].product_option_name)
					console.log("data[0].product_option_price: " + data[0].product_option_price)
					
					for (var i=0;i<data_length;i++) {
						console.log(data[i]);
						console.log(data[i].product_option_name);
						console.log(data[i].product_option_price);
					}
			...
			<div id="myCartList">
								...
								<c:forEach var="cart" items="${cartList }">
									<c:forEach var="product" items="${productList }">
										<c:if test="${cart.product_id == product.product_id }">
											...
											<tr class="cart_item">
												<td>
													<!-- cart_item의 product_id --> <input
													id="current_product_id" name="current_product_id"
													type="text" value="${product.product_id}" /> <select>
														<option>[필수] 옵션 선택</option>
														<c:forEach var="opt" items="${optList }">
															<c:if test="${ cart.product_id == opt.product_id}">
																<option value="${opt.product_option_price}">${opt.product_option_name}
																	(+
																	<fmt:formatNumber value="${opt.product_option_price}"
																		type="number" /> 원)
																</option>
															</c:if>
														</c:forEach>
												</select> <input type="checkbox" name="check_one">
												</td>
												<td><a
													href="${contextPath}/product/productDetail.do?product_id=${product.product_id}">
														<img alt="상품 이미지"
														src="${contextPath}/thumbnails.do?product_id=${cart.product_id}&fileName=${product.product_main_image}">
												</a></td>
												<td>
													<div class="cart_title">
														<a
															href="${contextPath}/product/productDetail.do?product_id=${product.product_id}">${product.product_name}</a>
													</div>
													<div class="cart_option">
														[옵션] ${cart.cart_option_name} (+
														<fmt:formatNumber value="${cart.cart_option_price}"
															type="number" />
														원)
													</div>
												<td class="modify_option_btn"><a href="javascript:" onClick="javascript:modifyPopup('open', '.layer01');">옵션변경</a>

												</td>
												<td class="product_total_price"><fmt:formatNumber
														value="${product.product_sales_price+cart.cart_option_price}"
														type="number" var="product_total_price" />
													${product_total_price }원</td>
												<td class="x_button"><a
													href="javascript:delete_cart_product('${cart.cart_id}');">X</a>
												</td>
											</tr>
										</c:if>
									</c:forEach>
				...
				<div id="modify_popup" style="visibility: hidden">
					...
					<form>
						...
						<div id="product_name">상품명 ${product.product_name}
							${cart.product_id}</div>
						<select id="cart_product_opt" name="cart_product_opt">
							<option value="0">[필수] 옵션 선택</option>
						</select> <a class="modify_button" href="#">저장</a>
					</form>
				</div>
			</div>
		</div>
